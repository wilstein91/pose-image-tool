...
3. 결과는 프로젝트 폴더의 `outputs/` 에 `output_{사진번호}_seed{seed}_{자세강도}_s{단계수}_g{충실도}.png` 이름으로 저장됩니다. 사진번호는 STEP 3-1에서 고른 `PICK` 을 따라갑니다 (`PICK = 0` → `output_01`). 같은 이름이 있으면 `-1`, `-2` 가 붙습니다. 번호만 쓰고 싶으면 STEP 9-1의 `WITH_SETTINGS` 를 `False` 로 바꾸세요.
...
- **간혹 결과에 알록달록한 뼈대 선이 그대로 나옵니다.** 모델이 참조 이미지를 그대로 베끼려 할 때 생깁니다. `POSE_STRICTNESS` 를 낮추고, 프롬프트 끝에 `Clean photographic scene, no diagram lines.` 를 덧붙이면 대부분 사라집니다.
